...
参考文献需要.bib文件中按照要求输入。具体这块内容大家请教研究生师兄和师姐。
使用时有以下几种常用形式：
...
（2）多位作者：张三\refcite{cairns1965,chen2001,ding2001}；
\begin{latex}
多位作者：张三\refcite{cairns1965,chen2001,ding2001}；
\end{latex}

（3）直接把参考文献作为正文的一个名词语法要素：文献\onlinecite{chen2001}。
\begin{latex}
直接把参考文献作为正文的一个名词语法要素：文献\onlinecite{chen2001}。
\end{latex}
...
在~\LaTeX~可以使用多种图形格式：.pdf，.jpg，.png，.eps等为目前常用格式。
...
单张图片插入形式如~\reffig{fig示例图1}~所示。
\begin{figure}[htbp]
\centering
\includegraphics[width = 0.8\textwidth]{示例图1.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{figure1nonliner}
\caption{非线性构形状态转移过程示意图}
\label{fig示例图1}%应放在\caption{}之后，否则引用时指向的是前一个插图
\end{figure}
\end{latex}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.3\textwidth]{清明}
\hspace{36pt}
\includegraphics[width=0.3\textwidth]{反复}
\caption{反清复明}
...
如果想要两幅并排的图片各有自己的标题，可以在 \texttt{figure} 环境中使用两个
 \texttt{minipage} 环境，每个环境里插入一个图，如图~\ref{fig:清明}~和
~\ref{fig:反复}~所示。
...
\begin{minipage}[t]{0.3\textwidth}
    \centering
    \includegraphics[width=\textwidth]{清明}
    \caption{清明}
    \label{fig:清明}
\end{minipage}
\hspace{36pt}
\begin{minipage}[t]{0.3\textwidth}
    \centering
    \includegraphics[width=\textwidth]{反复}
    \caption{反复}
    \label{fig:反复}
\end{minipage}
\end{figure}
...
\begin{tabular}{cccc}
\toprule
{\bf\winsongti 文献类型} & {\bf\winsongti 标识代码} & {\bf\winsongti 文献类型} & {\bf\winsongti 标识代码}\\
\midrule
普通图书 & M &  会议录 & C\\
汇编 & G & 报纸 & N\\
期刊 & J & 学位论文 & D\\
...
\multicolumn{3}{c}{{\bf\winsongti 续表~\thetable\hskip1em} {\bf\winsongti 中国省级行政单位一览}}\\
\toprule {\bf\winsongti 名称} & {\bf\winsongti 简称} & {\bf\winsongti 省会或首府}  \\ \midrule
\endhead
\caption{中国省级行政单位一览}
\label{table2}\\
\toprule {\bf\winsongti 名称} & {\bf\winsongti 简称} & {\bf\winsongti 省会或首府}  \\ \midrule

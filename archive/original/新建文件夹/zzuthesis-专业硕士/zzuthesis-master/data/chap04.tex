...
\begin{equation}
\label{eq:1}
\ddot{\bf{\rho}}-\frac{\mu}{R_t^3}\left(3\bf{R_t}\frac{\bf{R_t\rho}}{R_t^2}-\bf{\rho}\right)=\bf{a}
\end{equation}
\begin{flushleft}
\renewcommand\arraystretch{1.25}
\begin{tabularx}{\textwidth}{@{}>{\normalsize\rm}l@{\quad}>{\normalsize\rm}l@{——}>{\normalsize\rm}X@{}}
式中& $\bf{\rho}$ &追踪飞行器与目标飞行器之间的相对位置矢量；\\
&  $\ddot{\bf{\rho}}$&追踪飞行器与目标飞行器之间的相对加速度；\\
&  $\bf{a}$   &推力所产生的加速度；\\
&  $\bf{R_t}$ & 目标飞行器在惯性坐标系中的位置矢量；\\
&  $\omega_{t}$ & 目标飞行器的轨道角速度；\\
&  $\bf{g}$ & 重力加速度，$=\frac{\mu}{R_{t}^{3}}\left(
3\bf{R_{t}}\frac{\bf{R_{t}\rho}}{R_{t}^{2}}-\bf{\rho}\right)=\omega_{t}^{2}\frac{R_{t}}{p}\left(
3\bf{R_{t}}\frac{\bf{R_{t}\rho}}{R_{t}^{2}}-\bf{\rho}\right)$，这里~$p$~是目标飞行器的轨道半通径。
...
\begin{equation*}
\left. \begin{aligned}
  \bf{B'}&=-\bf{\partial}\times \bf{E},\\
  \bf{E'}&=\bf{\partial}\times \bf{B} - 4\pi j,
\end{aligned}
\right\}
